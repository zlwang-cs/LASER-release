...
class BertPreTrainedForSeq2SeqModel(BertPreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, reuse_position_embedding=None,
        ...
        if model_type == 'layoutlm':
            for k in list('xyhw'):
                k = f'bert.embeddings.{k}_position_embeddings.weight'
                config_dim = config.max_2d_position_embeddings
                state_dim = state_dict[k].data.shape[0]
                feat_dim = state_dict[k].data.shape[1]
                if config_dim > state_dim:
                    logger.info('2D Pos: Resize > 2d position embedding !')
                    new_position_embedding = state_dict[k].data.new_tensor(torch.zeros(
                        size=(config_dim, feat_dim)), dtype=torch.float)
                    new_position_embedding = nn.Parameter(data=new_position_embedding, requires_grad=True)
                    new_position_embedding.data.normal_(mean=0.0, std=config.initializer_range)
                    new_position_embedding[:state_dim, :] = state_dict[k].data

                    state_dict[k] = new_position_embedding.data
                    del new_position_embedding
                elif config_dim != state_dim:
                    raise AttributeError

        useless_para_name = 'crit_mask_lm_smoothed.one_hot'
        if useless_para_name in state_dict:
            state_dict.pop(useless_para_name)
...
class BertModel(BertPreTrainedForSeq2SeqModel):
    ...
    def get_extra_token_embeddings(self, ids, xys, token_type_id, with_layout=True):
        token_type_ids = torch.ones_like(ids) * token_type_id
        embedding_output = self.embeddings(
            ids, position_ids=None, token_type_ids=token_type_ids
        )
...
class LayoutlmModel(BertPreTrainedForSeq2SeqModel):
    ...
    def get_extra_token_embeddings(self, ids, xys, token_type_id, with_layout=True):
        ...
        # option = os.environ.get('DATASET', 'funsd')
        # if 'nolayout' in option:
        #     xys=None
        if not with_layout:
            xys = None
        
        embedding_output = self.embeddings(
            ids, xys, position_ids=None, token_type_ids=token_type_ids
        )
        return embedding_output
...
class LayoutlmForSequenceToSequence(BertPreTrainedForSeq2SeqModel):
    def __init__(self, config):
        ...
        # NOTE: additional classes for sp tags
        self.extra_token_cls = LayoutlmSPOnlyMLMHead(config, src_len=get_extra_token_num())

        self.extra_token_input_ids = \
            torch.tensor([get_extra_token_tokenizer_index(s) for s in EXTRA_TOKEN_LIST], dtype=torch.long)
            # torch.tensor([EXTRA_TOKEN_TOKENIZER_INDEX_DICT[s] for s in EXTRA_TOKEN_LIST], dtype=torch.long)
        if config.base_model_type == 'layoutlm':
            ...
            # torch.tensor([EXTRA_TOKEN_2D_POS_DICT[s] for s in EXTRA_TOKEN_LIST], dtype=torch.long)
        ...
        if config.label_smoothing > 0:
            self.crit_mask_lm_smoothed = LabelSmoothingLoss(
                config.label_smoothing, config.max_source_length + get_extra_token_num(), ignore_index=0, reduction='none')
    ...
    def forward(self, source_idxys, target_idxys, target_index,
                pseudo_idxys, num_source_tokens, num_target_tokens,
        ...
        bsz = source_embedding.shape[0]
        extra_token_batch_ids = self.extra_token_input_ids.unsqueeze(0).repeat(bsz, 1).to(input_ids.device)
        if self.extra_token_input_xys is not None:
            extra_token_batch_xys = self.extra_token_input_xys.unsqueeze(0).repeat(bsz, 1, 1).to(input_ids.device)
        else:
            extra_token_batch_xys = None

        extra_token_embedding = self.bert.get_extra_token_embeddings(ids=extra_token_batch_ids,
                                                                     xys=extra_token_batch_xys,
                                                                     token_type_id=self.source_type_id,
                                                                     with_layout=self._config.extra_token_with_layout)
        ...
        source_part_scores = self.cls(pseudo_sequence_output, source_embedding)
        extra_token_scores = self.extra_token_cls(pseudo_sequence_output, extra_token_embedding)

        # NOTE: src/sp token cls loss
        extra_token_cls_labels = (target_index >= source_len).long()
        extra_token_cls_scores = self.classifier(pseudo_sequence_output)
        extra_token_cls_loss = F.cross_entropy(extra_token_cls_scores.transpose(1, 2).float(),
                                               extra_token_cls_labels,
                                               reduction='none')
        extra_token_cls_loss = loss_mask_and_normalize(extra_token_cls_loss.float(), target_mask)
        ...
        if self.crit_mask_lm_smoothed:
            masked_lm_loss = self.crit_mask_lm_smoothed(
                F.log_softmax(prediction_scores.float(), dim=-1), target_index)
        else:
            masked_lm_loss = self.crit_mask_lm(
                prediction_scores.transpose(1, 2).float(), target_index)
        pseudo_lm_loss = loss_mask_and_normalize(
            masked_lm_loss.float(), target_mask)

        loss = pseudo_lm_loss + extra_token_cls_loss
