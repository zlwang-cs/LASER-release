...
class Seq2seqDatasetForLayoutlm(torch.utils.data.Dataset):
    ...
    def __getitem_layout__(self, idx):
        ...
        # target_tags = self.__trunk(feature['target_tags'], self.max_target_len, simple=True, value=self.tag_sp_id)
        ...
        # target_tags = self.__pad(target_tags, self.max_target_len, simple=True, value=self.tag_sp_id)
        # target_index = self.__clip_index(target_index)
...
def load_and_cache_line_order_examples(
    ...
    if cached_features_file is not None and os.path.exists(cached_features_file) and False:
        ...
        for layout in tqdm.tqdm(examples):
            ...
            src_ids = [tokenizer.convert_tokens_to_ids([str(tmp_i)])[:1] + src_layout for tmp_i, src_layout in
                       enumerate(layout['src'])]
            tgt_ids = [tokenizer.convert_tokens_to_ids([str(tmp_i)])[:1] + tgt_layout for tmp_i, tgt_layout in
                       enumerate(layout['tgt'])]
            tgt_index = layout['tgt_index']
...
def load_and_cache_layoutlm_examples(
        example_path, tokenizer, local_rank, cached_features_file, max_src_length=1024,
        layout_flag=True, shuffle=True, white_list=None
):
    # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    if local_rank not in [-1, 0]:
        torch.distributed.barrier()
    ...
    for text_file, layout_file in zip(text_files, layout_files):
        with open(text_file, mode='r', encoding='utf-8') as text_reader, \
            ...
            for i, (text_line, layout_line) in enumerate(zip(text_reader, layout_reader)):
                ...
                text = json.loads(text_line)
                layout = json.loads(layout_line)
                
                if white_list is not None and text['filename'] not in white_list:
                    continue
    ...
    def tokenize_text_and_layout_src(_text, _layout, _layout_flag):
        ...
        for i, (word, box) in enumerate(zip(words, _layout)):
            ...
            for token in tokens:
                if _layout_flag:
                    if box[0] == -1:
                        box = get_extra_token_2d_pos(token)
    ...
    def tokenize_text_and_layout_tgt(_text, _layout, _index, _index_split, _layout_flag):
        # if _text is None or _layout is None:
        #     return None, None
        ...
        for word, box, i in zip(words, _layout, _index):
            ...
            if isinstance(i, int):
                ...
                for token in tokens:
                    if _layout_flag:
                        box = get_extra_token_2d_pos(tokenizer.convert_ids_to_tokens(token))
                        ret.append([token] + box)
                    else:
                        ret.append(token)
                    ii = max_src_length + get_extra_token_predict_bias(tokenizer.convert_ids_to_tokens(token))
                    # ii = max_src_length + EXTRA_TOKEN_PREDICT_BIAS_DICT[tokenizer.convert_ids_to_tokens(token)]
                    ret_index.append(ii)
...
def load_and_cache_pure_text_examples(
        example_path, tokenizer, local_rank, cached_features_file, max_src_length=1024, shuffle=True, white_list=None
):
    # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    if local_rank not in [-1, 0]:
        torch.distributed.barrier()
    ...
    for text_file in text_files:
        with open(text_file, mode='r', encoding='utf-8') as text_reader:
            logger.info(f'Start loading {text_file}')
            for i, text_line in enumerate(text_reader):
                if (i + 1) % 10000 == 0:
                    logger.info(f'{i + 1} lines ...')
                text = json.loads(text_line)
                ...
                examples.append((json.loads(text_line)))

    features = []

    def tokenize_text_and_layout_src(_text):
        ret = []
        index_split = {}
        words = _text.split()
        # note: (OLD) the index should start from 1: 0-the cls token in src
    ...
    def tokenize_text_and_layout_tgt(_text, _index, _index_split):
        ret = []
        ret_index = []
        words = _text.split()
        for word, i in zip(words, _index):
            tokens = tokenizer.tokenize(word)
            tokens = tokenizer.convert_tokens_to_ids(tokens)
    ...
    for text in tqdm.tqdm(examples):

        src_ids, src_index_split = tokenize_text_and_layout_src(text['src'])
        tgt_ids, tgt_index = tokenize_text_and_layout_tgt(text['tgt'], text['tgt_index'], src_index_split)
