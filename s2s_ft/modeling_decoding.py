...
class LayoutlmEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, bbox, token_type_ids=None, position_ids=None, task_idx=None):
        ...
        if bbox is not None:
            assert (bbox[:, :, 3] < bbox[:, :, 1]).int().sum() == 0
...
class BertModelIncr(BertModel):
    ...
    def get_sp_strings_embeddings(self, ids, xys, token_type_id, task_idx, with_layout=True):
        token_type_ids = torch.ones_like(ids) * token_type_id
        embedding_output = self.embeddings(
            ids, position_ids=None, token_type_ids=token_type_ids, task_idx=task_idx)
...
class LayoutlmModelIncr(LayoutlmModel):
    ...
    def get_sp_strings_embeddings(self, ids, xys, token_type_id, task_idx, with_layout=True):
        ...
        embedding_output = self.embeddings(
            ids, xys, position_ids=None, token_type_ids=token_type_ids, task_idx=task_idx)
        return embedding_output
...
class LayoutlmForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
        ...
        self.extra_token_with_layout = extra_token_with_layout
        ...
        # NOTE: the num_labels is useless, just for another task
        self.cls = LayoutlmSPPreTrainingHeads(
            config, src_len=config.max_source_length, num_labels=num_labels)
        # NOTE: the new prediction head for sp tags.
        self.extra_token_cls = LayoutlmSPPreTrainingHeads(
            config, src_len=get_extra_token_num(), num_labels=num_labels
        )
        self.extra_token_input_ids = \
            torch.tensor([get_extra_token_tokenizer_index(s) for s in EXTRA_TOKEN_LIST], dtype=torch.long)
        
        if config.base_model_type == 'layoutlm':
            self.extra_token_input_xys = \
                torch.tensor([get_extra_token_2d_pos(s) for s in EXTRA_TOKEN_LIST], dtype=torch.long)
        else:
            self.extra_token_input_xys = None
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None, 
        ...
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx,
                                    mask_qkv=mask_qkv, next_token_mask_table=next_token_mask_table)
        ...
        while next_pos < output_length:
            ...
            if src_embedding is None:
                ...
                bsz = src_embedding.shape[0]
                sp_string_batch_ids = self.extra_token_input_ids.unsqueeze(0).repeat(bsz, 1).to(input_ids.device)
                if self.extra_token_input_xys is not None:
                    sp_string_batch_xys = self.extra_token_input_xys.unsqueeze(0).repeat(bsz, 1, 1).to(input_ids.device)
                else:
                    sp_string_batch_xys = None
                sp_string_embedding = self.bert.get_sp_strings_embeddings(ids=sp_string_batch_ids,
                                                                          xys=sp_string_batch_xys,
                                                                          token_type_id=self.source_type_id,
                                                                          task_idx=task_idx,
                                                                          with_layout=self.extra_token_with_layout)

            last_hidden = new_encoded_layers[-1][:, -1:, :]

            source_part_scores, _ = self.cls(last_hidden, None, src_embedding, task_idx=task_idx)
            extra_token_scores, _ = self.extra_token_cls(last_hidden, None, sp_string_embedding, task_idx=task_idx)

            # NOTE: whether extra token weights
            extra_token_cls_scores = self.classifier(last_hidden)
            extra_token_cls_logits = F.softmax(extra_token_cls_scores, dim=-1)

            source_part_size = source_part_scores.shape[-1]
            source_part_weights = extra_token_cls_logits[:, :, 0].unsqueeze(-1).repeat(1, 1, source_part_size)
            extra_token_size = extra_token_scores.shape[-1]
            extra_token_weights = extra_token_cls_logits[:, :, 1].unsqueeze(-1).repeat(1, 1, extra_token_size)

            prediction_weights = torch.cat([source_part_weights, extra_token_weights], dim=-1)
            prediction_scores = torch.cat([source_part_scores, extra_token_scores], dim=-1)
            prediction_scores *= prediction_weights
            
            prediction_scores = torch.softmax(prediction_scores, dim=-1)

            # NOTE: seq tag weights
            # seq_tag_scores = self.seq_tag_cls(last_hidden)
            # _, seq_tag_preds = torch.max(seq_tag_scores, dim=-1)
            # output_seq_tags.append(seq_tag_preds)

            _, max_ids = torch.max(prediction_scores, dim=-1)
            output_ids.append(max_ids)
            output_score_traces.append(prediction_scores)
            ...
            if not self.layout_flag:
                ...
                extra_token_idxys = torch.cat([self.extra_token_input_ids.unsqueeze(-1), self.extra_token_input_xys], dim=-1)
                extra_token_idxys = extra_token_idxys.unsqueeze(0)
                bsz = input_ids.shape[0]
                extra_token_idxys = extra_token_idxys.repeat(bsz, 1, 1).to(input_ids.device)
                extended_input_ids = torch.cat([input_ids, extra_token_idxys], dim=1)

                curr_ids = torch.gather(extended_input_ids, 1, index)
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None,
        ...
        while next_pos < output_length:
            ...
            if src_embedding.shape[0] != new_embedding.shape[0]:
                ...
                sp_string_embedding = first_expand(sp_string_embedding)
            ...
            if curr_max_ids is not None and next_token_mask_table is not None:
                next_token_mask = next_token_mask_table(curr_max_ids)
                prediction_scores *= next_token_mask
            ...
            beam_masks.append(torch.eq(k_ids, -1).type_as(kk_scores))
            ...
            curr_max_ids = max_ids

            if len(input_ids.shape) == 2:
                expand_input_ids = first_expand(input_ids)
                index = max_ids
                
                extra_token_ids = self.extra_token_input_ids.unsqueeze(0)
                bsz = expand_input_ids.shape[0]
                extra_token_ids = extra_token_ids.repeat(bsz, 1).to(expand_input_ids.device)
                extended_input_ids = torch.cat([expand_input_ids, extra_token_ids], dim=1)
                ...
                extra_token_idxys = extra_token_idxys.unsqueeze(0)
                bsz = expand_input_ids.shape[0]
                extra_token_idxys = extra_token_idxys.repeat(bsz, 1, 1).to(expand_input_ids.device)
                extended_input_ids = torch.cat([expand_input_ids, extra_token_idxys], dim=1)
        ...
        # back tracking
        traces = {'pred_seq': [], 'wids': [], 'ptrs': [], 'pred_scores': []}
        for b in range(batch_size):
            # [(beam,)]
            ...
            if frame_id == -1:
                ...
                traces['pred_scores'].append([0])
            else:
                seq = [wids_list[frame_id][pos_in_frame]]
                score_seq = [scores[frame_id][pos_in_frame]]
                for fid in range(frame_id, 0, -1):
                    pos_in_frame = ptrs[fid][pos_in_frame]
                    seq.append(wids_list[fid - 1][pos_in_frame])
                    score_seq.append(np.exp(scores[fid - 1][pos_in_frame]))
                seq.reverse()
                score_seq.reverse()
                traces['pred_seq'].append(seq)
                traces['pred_scores'].append(score_seq)
        ...
        for k in ('pred_seq', 'wids', 'ptrs', 'pred_scores'):
            ts_list = traces[k]
            if not isinstance(ts_list[0], torch.Tensor):
                dt = torch.float if k in ['scores', 'pred_scores'] else torch.long
